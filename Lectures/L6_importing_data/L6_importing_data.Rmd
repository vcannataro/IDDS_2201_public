...
subtitle: "<br><br> IDDS 2201: Data Analytics, Module 6"
...
```{r message=FALSE}
boston_airbnb_badnames <- read_csv("data/boston_airbnb_badnames.csv")
names(boston_airbnb_badnames)
```
...
```{r error=TRUE}
ggplot(boston_airbnb_badnames, aes(x = Number of bathrooms, y = Price)) +
  geom_point()
```
...
.small[
```{r}
boston_airbnb <- read_csv("data/boston_airbnb_badnames.csv",
...
## Option 2: Load in the data and then format

```{r}
boston_airbnb <- read_csv("data/boston_airbnb_badnames.csv")
names(boston_airbnb) # wow gross
boston_airbnb %>%
  rename(bathrooms = `Number of bathrooms`,
...
## **Other packages** and the types of data they parse
...
### .hand[Your turn!]
.midi[
- ECLearn > Assignments > `in-class exercises, Nobels and sales + Data import` > `sales-excel.Rmd`. 
